...
export default class UploadController {
  ...
  async uploadMultipleFilesLocal(@UploadedFiles() files: any[]) {
    const result = [];
    if (ShareFunction.env().STORAGE_SERVER === 's3') {
      for (let i = 0; i < files.length; i += 1) {
        let path = files[i].location;

        if (!files[i].location?.startsWith('https://')) {
          path = `https://${path}`;
        }
  ...
  @HttpCode(201)
  @UseInterceptors(StorageFileInterceptor({ fieldName: 'file' }))
  async convertDocumentToText(@UploadedFile() file: any) {
    return { files: file };
  }
  ...
  async confirmFilesWasUsedLocal(
    ...
    if (ShareFunction.env().STORAGE_SERVER === 's3') {
      ...
      const filesWasUsedPromise = files.map((file) => {
        return this.uploadS3Service.confirmFileWasUsed(
