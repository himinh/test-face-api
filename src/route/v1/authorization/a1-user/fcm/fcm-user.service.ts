...
export default class FcmUserService {
  ...
  public async pushFCMToUsers(
    ...
    users.forEach((user) => {
      ...
      } else if (user.isEnableFCM && user.deviceID !== '') {
    ...
    if (resultSend && resultSend.responses)
      await this.updateFCMTokenValid(updateUsers, tokens, resultSend.responses);
  }
  ...
  private async saveUsersFCMTokenValid(users: UserDocument[]) {
    const updatedUsersPromise = users.map((user) =>
      this.userRepository.updateFcmTokensById(user._id, user.fcmTokens),
    );
