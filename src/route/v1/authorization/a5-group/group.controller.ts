...
export default class GroupController {
  ...
  async removeGroupDetails(
    ...
    const removePromise = body.groupDetails.map((groupDetail) =>
      this.groupService.removeGroupDetail(id, groupDetail),
    );
  ...
  async addGroupDetails(
    ...
    const updatePromise = body.groupDetails.map((groupDetail) =>
      this.groupService.addGroupDetail(id, groupDetail),
    );
  ...
  async addUsers(
    ...
    const updatePromise = body.users.map((userId: string) =>
      this.userService.updateGroup(new Types.ObjectId(userId), id, options),
    );
  ...
  async deleteManyByIds(@Param('ids') ids: string): Promise<any> {
    const listIds = ids.split(',').map((item: any) => new Types.ObjectId(item));
