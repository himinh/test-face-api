...
export default class StorageService {
  ...
  async uploadFileToS3(
    ...
    return this.s3Service?.uploadFile(params, options);
  }
   ...
   * @returns
   */
  async uploadGooglePhoto(buffer: Buffer): Promise<any> {
    const result = await this.uploadFileToS3({
      Bucket: `${ShareFunction.env().S3_BUCKET_NAME_PATH}`,
      Body: buffer,
      ACL: 'public-read',
      Key: `google-photos/${generateCodeID()}_${new Date().getTime()}_${generateCodeID()}.jpg`,
    });
   ...
   * @returns
   */
  async getFileS3(params: GetObjectRequest): Promise<any> {
    return this.s3Service?.getFile(params);
  }
   ...
   * @returns
   */
  async uploadFile(outputPath: string): Promise<any> {
    const buffer = readFileSync(outputPath);
    const key = outputPath.split('/').slice(-2).join('/');
